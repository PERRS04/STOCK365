            ...
            <div class="p-4">
                ...
                <div class="flex items-start justify-between gap-2 mb-3">
                    ...
                    <div class="flex items-center gap-1.5 shrink-0 mt-0.5">
                        ...
                        @else
                        <span class="w-[6px] h-[6px] rounded-full {{ $dotClass }} shrink-0"></span>
                        @endif
                ...
                <div class="grid grid-cols-2 gap-x-3 gap-y-2 mb-3">
                    ...
                    {{-- Operators online --}}
                    <div>
                        <p class="text-[8px] font-bold uppercase tracking-[0.12em] text-gray-400 mb-0.5">Ops.</p>
                        <p class="text-[16px] font-bold tabular-nums leading-none {{ $node['operators']->count() > 0 ? 'text-blue-500' : 'text-gray-300' }}">
                ...
                {{-- Operator name tags --}}
                @if($node['operators']->isNotEmpty())
                <div class="flex flex-wrap gap-1">
                    @foreach($node['operators']->take(3) as $op)
                    <span class="text-[9px] font-medium text-gray-500 bg-gray-100 px-1.5 py-[2px] rounded-md leading-none truncate max-w-[80px]">{{ $op }}</span>
                    @endforeach
                    @if($node['operators']->count() > 3)
                    <span class="text-[9px] text-gray-400 bg-gray-50 px-1.5 py-[2px] rounded-md leading-none">+{{ $node['operators']->count() - 3 }}</span>
                    @endif
                </div>
                @elseif($isOffline)
