...
<div class="space-y-5">
    ...
    <div class="flex items-center justify-between">
        <div>
            ...
            <p class="text-[12px] text-gray-400 mt-0.5">{{ $almacen->nombre }} · {{ $movements->total() }} registros</p>
        </div>
    </div>
        ...
        <table class="w-full text-[13px]">
            ...
            <tbody class="divide-y divide-gray-50">
                ...
                @php
                    ...
                    $isPositive = $mov->tipo === 'entrada' || ($mov->tipo === 'ajuste' && $mov->cantidad > 0);
                    $isNegative = in_array($mov->tipo, ['salida', 'pérdida', 'transferencia'])
                ...
                <tr class="hover:bg-gray-50/50">
                    ...
                    <td class="px-5 py-3 text-[12px] text-gray-400">
                        @if($mov->reference_type === 'transfer' && $mov->reference_id)
                            <a href="{{ route('deposito.transferencias.show', $mov->reference_id) }}"
                               class="text-stock-primary hover:underline">#Transf. {{ $mov->reference_id }}</a>
                        @else
                            —
                        @endif
                    </td>
                ...
                @empty
                <tr>
                    <td colspan="6" class="px-5 py-10 text-center text-[13px] text-gray-400">Sin movimientos registrados</td>
                </tr>
                @endforelse
            </tbody>
        </table>
