@extends('layouts.app')
@section('title', 'Mis Transferencias')
@section('content')
...
<div class="space-y-5">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-[18px] font-semibold text-gray-900">Transferencias</h1>
            <p class="text-[12px] text-gray-400 mt-0.5">{{ $almacen->nombre }} · solicitudes de stock</p>
        </div>
        <a href="{{ route('deposito.transferencias.create') }}"
           class="flex items-center gap-1.5 px-4 py-2 bg-stock-primary text-white text-[13px] font-medium rounded-lg hover:bg-stock-primary/90 transition">
    ...
    <div class="flex gap-2">
        ...
        <a href="{{ route('deposito.transferencias', $val ? ['estado' => $val] : []) }}"
           class="flex items-center gap-2 px-4 py-2 rounded-xl border text-[12px] font-medium transition
                  {{ request('estado', '') === $val
                     ? match($val) {
                         ...
                         'aprobado'  => 'bg-emerald-50 border-emerald-300 text-emerald-800',
                         'rechazado' => 'bg-red-50 border-red-300 text-red-800',
        ...
        <table class="w-full text-[13px]">
            ...
            <tbody class="divide-y divide-gray-50">
                ...
                <tr class="hover:bg-gray-50/50">
                    ...
                    <td class="px-5 py-3 font-medium text-gray-900">{{ $transfer->fromLocationName() }}</td>
                    <td class="px-5 py-3 text-gray-700">→ {{ $transfer->toLocationName() }}</td>
                    <td class="px-5 py-3 text-gray-500">{{ $transfer->createdBy?->name ?? '—' }}</td>
                    <td class="px-5 py-3 text-center">
                        <span class="inline-flex items-center px-2 py-0.5 rounded-full border text-[11px] font-medium {{ $stateCls }}">
                            {{ $stateLabel }}
                        </span>
                    </td>
                    <td class="px-5 py-3 text-right">
                        <a href="{{ route('deposito.transferencias.show', $transfer) }}"
                           class="text-[12px] font-medium text-stock-primary hover:underline">Ver →</a>
                    </td>
                </tr>
                @empty
                <tr>
